...
import { useGetAllTicketsQuery, useGetAllUsersQuery, useGetUsersListQuery, useUpdateTicketHasWonMutation } from '../features/apiSlice'
...
const Transactions = () => {
    ...
    const [updateTicketHasWon] = useUpdateTicketHasWonMutation();
    ...
    const handleUpdateHasWon = async (ticketId, gameId, hasWon) => {
        try {
            await updateTicketHasWon({ ticketId, gameId, hasWon }).unwrap();
